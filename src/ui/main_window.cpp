    ...
    while (running_.load()) {
        ...
        for (const auto& f : faces) {
            ...
            if (w > config::MIN_FACE_WIDTH_TRIGGER) {
                ...
                // Pause detection for the cooldown period so we don't
                // trigger recognition on every frame after a capture.
                std::this_thread::sleep_for(
                    std::chrono::duration<double>(config::RECOGNITION_COOLDOWN_SEC));
                break; // No need to examine more faces this frame
            }
        }
    }
}
